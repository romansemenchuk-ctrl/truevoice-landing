...
function protectionHeaders(c,oidcToken=''){const h={};const token=String(oidcToken||'');if(token)h['x-vercel-trusted-oidc-idp-token']=token;else if(c.vercelBypass)h['x-vercel-protection-bypass']=c.vercelBypass;return h;}
async function ledgerCall(action,payload,{env=process.env,fetchImpl=globalThis.fetch,oidcToken=''}={}){
 ...
 try{
  ...
  const headers={'Content-Type':'application/json',Authorization:'Bearer '+c.secret,...protectionHeaders(c,oidcToken)};
...
async function ledgerProbe({env=process.env,fetchImpl=globalThis.fetch,oidcToken=''}={}){
 const c=config(env);
 if(c.mode==='off'||!c.url||!c.secret||c.secret.length<32)return false;
 const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),4000);
 try{
  const join=c.url.includes('?')?'&':'?';
  const headers={Authorization:'Bearer '+c.secret,...protectionHeaders(c,oidcToken)};
  const r=await fetchImpl(c.url+join+'action=bridge-health',{method:'GET',headers,signal:controller.signal});
  if(!r.ok)return false;
  let body=null;try{body=await r.json();}catch{return false;}
  return body?.ok===true;
 }catch{return false;}
 finally{clearTimeout(timer);}
}
module.exports={config,protectionHeaders,ledgerCall,ledgerProbe};
