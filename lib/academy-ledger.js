...
function config(env=process.env){const mode=String(env.ACADEMY_LEDGER_MODE||'off').toLowerCase();if(!MODES.has(mode))throw new Error('academy_ledger_bad_mode');return{mode,url:String(env.ACADEMY_LEDGER_URL||''),secret:String(env.ACADEMY_LEDGER_SECRET||'')};}
async function ledgerCall(action,payload,{env=process.env,fetchImpl=globalThis.fetch}={}){
 ...
 try{
  const join=c.url.includes('?')?'&':'?';const r=await fetchImpl(c.url+join+'action='+encodeURIComponent(action),{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+c.secret},body:JSON.stringify(payload),signal:controller.signal});
